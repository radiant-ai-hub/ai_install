...
- For day-to-day Windows work with `uv` and Python, Git Bash is the preferred interactive shell. The installer runs from PowerShell.
...
- On Windows ARM, the installer uses `uv` to get the native ARM64 Python `3.13.12` build
...
- On Windows, the installer adds a Git Bash profile to Windows Terminal when a Windows Terminal settings file is present.
- The macOS installer downloads tools directly from official vendor sources, similar to `radiant_install`.
...
- The default managed Python version is pinned to `3.13.12`
